...
compiled/                      # Generated by build.sh
└── wallpaper-*.html          # Single-file builds for Hidamari
```

### Required Files per Theme

Each theme folder under `theme/themes/` must contain:

1. **`theme.json`** (required) - Theme colors and configuration
2. **`background.js`** (required) - Custom background animation
3. **`openbar-theme-config`** (optional) - Desktop shell colors for GNOME OpenBar extension
...
```bash
# Build single-file wallpapers (outputs to compiled/ in project root)
./build.sh

# Test in browser (multi-file mode with dynamic loading)
xdg-open theme/base/background.html

# Test Hidamari mode (single-file, no API)
xdg-open compiled/wallpaper-my-theme.html

# Install to system location
sudo ./install.sh
# Compiled files will be in ~/WallpagesThemes/compiled/
```
...
2. **Test with OpenBar:**
   ...
   - Import theme config: `~/WallpagesThemes/themes/my-theme/openbar-theme-config`
...
- [Example Themes](../../theme/themes/) - Browse all 12 available theme implementations
- [API Documentation](api.md) - Backend API integration
- [Architecture](../architecture/overview.md) - System design and diagrams
- [Hidamari Integration](hidamari.md) - Single-file wallpaper compilation for Hidamari
